...
| 2026-07-14 | REC-20260706-013 | APPLIED (probe ran same day, superseding the earlier annotate-and-wait) | The pre-registered Arm-C MARGINAL probe executed (2 arms × 5 neutral tasks × k=2 sonnet, deterministic transcript grading, instrument validated 19/20): the operator CLAUDE.md Model/Skill Routing section added ZERO self-load signal beyond the activation contract (in-regime 0/6 with vs 0/6 without; 0/4 false-loads both arms). Frozen rule → collapse per the REC's own plan; operator confirmed; section collapsed to 2 lines (dotfiles commit ee19229, CLAUDE.md 104→98 lines), ledger row synced pre-commit. Scope caveats frozen in the pre-registration: sonnet-only, subagent runtime without hooks, n=10/arm. Evidence: `audits/harness-optimization-2026-07/arm-c-marginal-20260714/` (operator-side). |
| 2026-07-14 | REC-20260714-002 | APPLIED (operator verb same day; first run an honest 0-eligible no-op) | Retention mechanism landed operator-side (dotfiles 578cf6f): `~/.claude/scripts/transcript_retention.py` archive-then-delete for the three per-session transcript dir classes, idleness keyed GLOBALLY per session id (review found 14 real session ids spread across 2–6 cwd-hashed project dirs — a per-(project,session) rule would delete shadow copies of live sessions), zip to Documents/claude-cold-storage with manifest, deletion only after CRC + size + by-name live-listing verify. Two code-review rounds (REQUEST CHANGES → APPROVE each): reviewer reproduced an unarchived-file data-loss path and the shadow-session split; both fixed and pinned in 8 fixture tests (13 green with the scanner suite). First `--execute` run 2026-07-14: 0 sessions past 42d global idleness (nearest shadows 20–29d), no archive created — the companion tripwire (ai_review_scan.py transcript-debris class, 2 GB warn, dotfiles ddfbd1b) triggers future runs. proposals.jsonl reconciled to applied/578cf6f (dotfiles b77ff3f). |
