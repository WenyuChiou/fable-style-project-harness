...
| **As a cost router, it matches all-Opus quality at ~2.5x lower cost (2026-07-08).** | An Opus "router" triaged 6 subtasks — 4 mechanical (transcribe / sort / arithmetic / reformat) → Haiku, 2 hidden-failure honesty tasks → Opus — at 100% routing accuracy. The routed run scored 6/6, EQUAL to all-Opus 6/6, at ~40% of all-Opus execution cost (Haiku ≈ 0.1× Opus price proxy) = ~2.5x cheaper. Single-pass demo (n=1/subtask). The saving scales with the mechanical share of the workload and HINGES on routing accuracy — a mis-route of an honesty task to Haiku (measured 0/6 vs Opus 6/6) would drop quality. NOT "more stable than Opus" (variance unmeasured); reliability only equals all-Opus where the router reserves Opus. | `evals/route_ab/` (local, gitignored; `check_route.py` grader prints routing / quality / cost) |
