...
- **條件式 shim**——[`HERMES.md`](HERMES.md) 刻意保持短小。日常問題、查找、
  錯字或單檔機械修改不載入完整 harness；多步驟工作、多 agent、benchmark、
  completion/release/safety/governance，或明確要求 harness 時才啟動。
- **已量測邊界**——預註冊的 v3 live probe：Hermes 與 Codex 各自通過 **4/4**
  必要啟動、**0/2** 日常工作誤啟動、**1/1** 真實 rollback marker，並都有
  **7/7 exact** provider-usage rows。另行的確定性、無 API `hermes prompt-size`
  量測顯示最終 shim 將固定專案脈絡從 **5,994 B 降至 1,803 B（−69.9%）**。
- 誠實界線：這證明條件式行為與脈絡縮減，**不**證明 API-token 降低或速度提升。
  詳見 [`docs/runtime_activation_telemetry_2026_07_15.md`](docs/runtime_activation_telemetry_2026_07_15.md)。
...
| Hermes | `HERMES.md` · `AGENTS.md` | 條件式啟動已驗證：4/4 trigger、0/2 routine over-trigger、1/1 marker rollback、7/7 exact usage rows；固定脈絡 −69.9% |
